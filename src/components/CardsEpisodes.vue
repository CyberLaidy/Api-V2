...
<template>
    ...
    <div class="field">
      <div class="control">
        <button class="button is-link" v-on:click="showCharacters">Search</button>
      </div>
    </div>
  </div>
  <!--  -->
    <p class="titulo-home">Episode List</p>
...
<!--                     <img v-bind:src="character.image" v-bind:alt="character.name" style="border-radius: 50px; width: 180px;">
 -->                  </div>
                    ...
                    <v-card-text class="texto-nombre-episode  my-4 text-center text-h6">Episode: {{ character.id }}</v-card-text>
                    <v-card-text class="texto-nombre-episode  my-4 text-center text-h6">Name: {{ character.name }}</v-card-text>
  ...
  export default{
    //accedemos a la propiedad personajes y damos nombre al componente.
    name: "CharacterItem",
    props: ["characters"]
  }
